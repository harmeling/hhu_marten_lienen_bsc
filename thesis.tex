...
\subsection{First Degree Polynomials}

Let $X$ be a random variable with a Gaussian distribution, i.e.
$X \sim \mathcal{N}(\mu, \sigma^{2})$. The most basic thing, you can do with
$X$, is plugging it into a first degree polynomial. Luckily every operation
involved is just a transformation of the parameters of the original
distribution.

The first two lemmata will establish rearrangement rules for Gaussian
distributions.
\begin{lemma}
...
\begin{lemma}
  ...
  Let $f \in \mathbb{R} \setminus \{ 0 \}$. Then
  $\mathcal{N}(fx \mid \mu, \sigma^{2}) = \frac{1}{|f|} \cdot \mathcal{N}\left(x \mid
  \frac{\mu}{f}, \frac{\sigma^{2}}{f^{2}}\right)$.
\end{lemma}
\begin{proof2}
  \begin{align*}
    \mathcal{N}(fx \mid \mu, \sigma^{2}) = & \frac{1}{\sqrt{2 \pi \sigma^{2}}} \exp\left( -\frac{\left(fx - \mu \right)^{2}}{2\sigma^{2}} \right)\\
    = & \frac{1}{\sqrt{2 \pi \left( \frac{\sigma f}{f} \right)^{2}}} \exp\left( -\frac{f^{2}\left(x - \frac{\mu}{f} \right)^{2}}{2\sigma^{2}} \right)\\
    = & \frac{1}{|f|} \frac{1}{\sqrt{2 \pi \frac{\sigma^{2}}{f^{2}}}} \exp\left( -\frac{\left(x - \frac{\mu}{f} \right)^{2}}{2 \frac{\sigma^{2}}{f^{2}}} \right)\\
    = & \frac{1}{|f|} \cdot \mathcal{N}\left( x \mid \frac{\mu}{f}, \frac{\sigma^{2}}{f^{2}} \right)
  \end{align*}
\end{proof2}
...
\begin{lemma}[Change of variables]
  ...
  hence invertible. Then we can ``unapply'' $f$ through the following formula
  \begin{equation*}
...
\begin{lemma}
  Let $f \in \mathbb{R}$. Then
  $f + X = X + f \sim \mathcal{N}(\mu + f, \sigma^{2})$.
\end{lemma}
\begin{proof2}
  This follows directly from lemma \ref{lemma:x-mu-symmetry} and
  \ref{lemma:change}.
  \begin{align*}
    p(X + f = x) = p(X = x - f) \cdot \left| \frac{\mathrm{d}(x - f)}{\mathrm{d}x} \right| = \mathcal{N}(x - f \mid \mu, \sigma^{2}) = \mathcal{N}(x \mid \mu + f, \sigma^{2})
  \end{align*}
\end{proof2}

\begin{corollary}
  Let $f \in \mathbb{R}$. Then $X - f \sim \mathcal{N}(\mu - f, \sigma^{2})$.
\end{corollary}
\begin{proof2}
  $X - f = X + (-f)$
\end{proof2}
...
\begin{corollary}
  Let $f \in \mathbb{R}$. Then $f - X \sim \mathcal{N}(f - \mu, \sigma^{2})$.
\end{corollary}
\begin{proof2}
  $f - X = f + (-X)$
\end{proof2}
...
\begin{lemma}
  Let $f \in \mathbb{R} \setminus \{ 0 \}$. Then
  $fX = Xf \sim \mathcal{N}(f\mu, f^{2}\sigma^{2})$.
\end{lemma}
\begin{proof2}
  ...
  \begin{align*}
    p(fX = x) & = \left| \frac{\mathrm{d}\frac{x}{f}}{\mathrm{d}x} \right| \cdot p\left( X = \frac{x}{f} \right)\\
              ...
              & = \left|\frac{1}{f}\right| \cdot \mathcal{N}\left(\frac{x}{f} \mid \mu, \sigma^{2}\right)\\
              & = \left|\frac{1}{f}\right| \cdot |f| \cdot \mathcal{N}\left(x \mid f\mu, f^{2}\sigma^{2}\right)\\
              & = \mathcal{N}\left(x \mid f\mu, f^{2}\sigma^{2}\right)
  \end{align*}
\end{proof2}

\begin{corollary}
  Let $f \in \mathbb{R} \setminus \{ 0 \}$. Then
  $\frac{X}{f} \sim \mathcal{N}\left( \frac{\mu}{f}, \frac{\sigma^{2}}{f^{2}}
  \right)$.
\end{corollary}
\begin{proof2}
...
Note, that we did not cover $\frac{f}{X}$. Division is not monotonic, so lemma
\ref{lemma:change} does not hold and the distribution of $\frac{f}{X}$ cannot be
evaluated with the techniques used in this section.
