...
Affine transformations are not terribly interesting, because they can easily be
calculated using symbolics. We will plot one anyway to demonstrate, that the
attached library can correctly compute it. In figure \ref{fig:affine} we apply
the affine transformation
\begin{equation*}
  f(a, b, c) = \frac{3a}{5} + 2b - c - 3
\end{equation*}
to the variables plotted in figure \label{fig:affine-vars}. The computed mixture
plotted in figure \ref{fig:affine-result} is perfectly consistent with the
sampled PDF as expected.
\begin{figure}[h]
  \centering
