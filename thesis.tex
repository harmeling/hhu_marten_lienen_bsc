...
hand.
\begin{equation*}
  X \sim \mathcal{N}(\mu, \sigma^{2}) \qquad Y \sim \mathcal{N}(\nu, \tau^{2}) \qquad \Rightarrow \qquad Z = X + Y \sim \mathcal{N}(\mu + \nu, \sigma^{2} + \tau^{2})
\end{equation*}
...
$\sum_{i} \alpha_{i} \cdot \mathcal{N}(\mu_{i}, \sigma^{2}_{i})$. Gaussian
...
\begin{appendices}
  ...
  Gaussian distribution, symbolized with $\mathcal{N}(\mu, \sigma^{2})$. It has
  two parameters, the mean $\mu$ and the variance $\sigma^{2}$. The former tells
  you, which value is the most probable, while the latter says, how fast numbers
  get less probable the farther they are from $\mu$.

  The notation $X \sim \mathcal{N}(\mu, \sigma^{2})$ means, that the random
  variable $X$ has the associated distribution $\mathcal{N}$ with parameters
  $\mu$ and $\sigma^{2}$. The function, that tells you, how high the probability
  density is at $x$, i.e. how probable $x$ is, is written $p(X = x)$. To
  determine this, $p$ plugs the value $x$ into the associated distribution of
  $X$. In this case we have
  \begin{equation*}
    p(X = x) = \mathcal{N}\left( x \mid \mu, \sigma^{2} \right) = \frac{1}{\sqrt{2 \pi \sigma^{2}}} \exp \left( - \frac{(x - \mu)^{2}}{2\sigma^{2}} \right)
  \end{equation*}
